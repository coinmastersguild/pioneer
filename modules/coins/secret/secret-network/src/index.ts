...
let URL_SECRET_LCD = process.env['URL_SECRET_LCD'] || 'https://lcd-secret.keplr.app'



let BASE_SECRET = 100000000
...
let broadcast_transaction = async function(tx:string){
    ...
    try{
        ...
        try{
            //push to seed
            let urlRemote = URL_SECRET_LCD+ '/txs'
...
let get_txs_by_address = async function(address:string){
    ...
    try{
        ...
        //sends
        let url = URL_SECRET_LCD+ '/txs?message.sender='+address
...
let get_node_info_verbose = async function(){
    ...
    try{
        ...
        //gaiad abci_info
        let nodeInfo = await axios({method:'GET',url: URL_SECRET_LCD+'/node_info'})
