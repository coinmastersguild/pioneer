...
module.exports = {
    ...
    getNftBalances: function (address:string, networkId:string) {
        return get_balances_nft(address,networkId);
    },
}

const get_balances_nft = async function (address:string,networkId:string) {
    ...
    try {
        ...
        const url = `https://api.covalenthq.com/v1/${chainId}/address/${address}/balances_nft/?with-uncached=true`;
...
const get_balances = async function (address:string,networkId:string) {
    ...
    try {
        ...
        if(networkId.indexOf('eip155') === -1) throw Error("EVM chains only! ex: eip155:1")
        let chainId = networkId.split(":")[1]
        const url = `https://api.covalenthq.com/v1/${chainId}/address/${address}/balances_v2/?with-uncached=true`;
