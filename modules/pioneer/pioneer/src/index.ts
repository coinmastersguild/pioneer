    ...
    constructor(type:HDWALLETS,config:Config,isTestnet?:boolean) {
        ...
        this.init = async function (wallet?:any) {
            ...
            try{
                ...
                this.txBuilder = new TxBuilder('',configBuilder);
                ...
                switch (+HDWALLETS[this.type]) {
                    case HDWALLETS.pioneer:
                        ...
                        log.info(tag,"config.mnemonic: ",config.mnemonic)
                        let resultLoad = await this.WALLET.loadDevice({ mnemonic: config.mnemonic })
                        log.info(tag,"resultLoad: ",resultLoad)

                        //check innited
                        let isInit = await this.WALLET.isInitialized()
                        ...
                        log.info(tag,"Checkpoint valid paths ** paths: ",paths)
                        this.pubkeys = await this.WALLET.getPublicKeys(paths)
                        if(!this.pubkeys) throw Error("Failed to get pubkeys!")
