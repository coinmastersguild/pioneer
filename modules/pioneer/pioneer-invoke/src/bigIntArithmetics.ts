...
const toMultiplier = (decimal: number): bigint => {
  ...
  try {
    const result = BigInt(10) ** BigInt(decimal);
...
export class BigIntArithmetics {
  decimalMultiplier: bigint = BigInt(10) ** BigInt(8);
  bigIntValue: bigint = BigInt(0);
  decimal?: number;
  ...
  getValue<T extends AllowedNumberTypes>(type: T): NumberPrimitivesType[T] {
    ...
    switch (type) {
      ...
      case 'bigint':
        return ((this.bigIntValue * BigInt(10) ** BigInt(this.decimal || 8)) /
